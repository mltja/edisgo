...
def calculate_curtailment(
    ...
    try:
        ...
        grid_results_dir = Path(os.path.join( # TODO
            grid_dir,
            "weekly_curtailment_benchmarking",
        ))
        ...
        files_check = [f for f in os.listdir(grid_results_dir) if day.strftime("%Y-%m-%d") in f]

        if len(files_check) >= 7:
            pass
        else:
            ...
            voltage_dev = results_helper_functions.voltage_diff(edisgo)
            issues = voltage_dev[
                abs(voltage_dev) > 3e-2].dropna( # TODO
                how="all").dropna(axis=1, how="all")
            if not issues.empty:
