...
def make_pseudo_coordinates(edisgo_root):
    ...
    def make_coordinates(graph_root):
        ...
        def coordinate_branch(pos_start, angle_offset, length, node_numerator, node_total_numerator):
            length = length / 1.3
        ...
        for i in len(list(nx.neighbors(graph_root, start_node))):
            ...
            path_to_max_distance_node = \
                list(nx.shortest_simple_paths(graph_copy, start_node, list(nx.nodes(graph_copy))[index]))[0]
        ...
        for node in list(nx.neighbors(graph_root, start_node)):
            n = n + 1
            pos, origin_angle = coordinate_source(graph_root.nodes[start_node]["pos"],
        ...
        while graph_copy.number_of_nodes() > 0:
            ...
            for node in list(nx.neighbors(graph_copy, next_node)):
                ...
                if node in path_to_max_distance_node:
                    ...
                    direction = math.fmod(len(list(nx.shortest_simple_paths(
                        graph_root, start_node, next_node))[0]), 2)
    ...
    start_time = time()
    logger.info('Start - Making pseudo coordinates for grid: {}'.format(str(edisgo_root.topology.mv_grid)))
...
def remove_one_meter_lines(edisgo_root):
    ...
    for index, row in lines_df.iterrows():
        if row.length < 0.001:
            logger.info('Line "{}" is {:.3f}m long and will not be removed.'.format(index, row.length * 1000))
        if row.length == 0.001:
            ...
            if (number_of_neighbors_bus0 != number_of_neighbors_bus1) \
                ...
                if (number_of_neighbors_bus0 > number_of_neighbors_bus1) and (
                    ...
                    busmap[row.bus1] = row.bus0
                        ...
                        number_of_neighbors_bus1 > number_of_neighbors_bus0) and number_of_neighbors_bus0 == 1:
                    unused_lines.append(index)
                    busmap[row.bus0] = row.bus1
            else:
                logger.info('Line "{}" is {:.3f}m long and will not be removed.'.format(index, row.length * 1000))
    logger.info('Drop {} of {} short lines ({:.0f}%)'.format(len(unused_lines), lines_df.shape[0], (len(unused_lines) / lines_df.shape[0] * 100)))
    ...
    buses_df = buses_df.groupby(by=['new_bus'], dropna=False, as_index=False, sort=False).first()
    ...
    for grid in grid_list:
        ...
        for index, row in lines_df.iterrows():
            if row.length < 0.001:
                ...
                distance_bus_0, path = nx.single_source_dijkstra(G, source=transformer_node, target=row.bus0, weight='length')
                distance_bus_1, path = nx.single_source_dijkstra(G, source=transformer_node, target=row.bus1, weight='length')

                logger.debug('Line "{}" is {:.5f}m long and will be removed.'.format(index, row.length * 1000))
                logger.debug('Bus0: {} - Distance0: {}'.format(row.bus0, distance_bus_0))
                logger.debug('Bus1: {} - Distance1: {}'.format(row.bus1, distance_bus_1))
...
def aggregate_to_bus(edisgo_root, aggregate_charging_points_mode=True):
    ...
    if not loads_df.empty:
        ...
        if load_aggregation_mode == 'sector':
            ...
            loads_df.index = 'Load_' + loads_df.loc[:, 'bus'] + '_' + loads_df.loc[:, 'sector']
        elif load_aggregation_mode == 'bus':
        ...
        load_name_map_df = edisgo_obj.topology.loads_df.loc[:, 'old_load_name'].to_dict()
        load_name_map = {}
        for i in range(0, len(load_name_map_df.keys())):
            for j in range(0, len(list(load_name_map_df.values())[i])):
                load_name_map[list(load_name_map_df.values())[i][j]] = \
                    list(load_name_map_df.keys())[i]
        ...
        edisgo_obj.timeseries.loads_active_power = timeseries_loads_p_df.groupby(level=0).sum().T
        edisgo_obj.timeseries.loads_reactive_power = timeseries_loads_q_df.groupby(level=0).sum().T

    if not generators_df.empty:
        logger.info('Aggregate generators_df')

        generators_df = generators_df.groupby(by=['bus', 'type', 'weather_cell_id'], dropna=False).apply(aggregate_generators)
        generators_df.index = ('Generator_' + generators_df.loc[:, 'bus'].values + '_' + generators_df.loc[:, 'type'].values + '_weather_cell_id_' + generators_df.loc[:, 'weather_cell_id'].apply(extract_weather_cell_id).values)

        edisgo_obj.topology.generators_df = generators_df

        logger.info('Aggregate generator timeseries')
        timeseries_generators_p_df = edisgo_obj.timeseries.generators_active_power.T.copy()
        timeseries_generators_q_df = edisgo_obj.timeseries.generators_reactive_power.T.copy()

        generator_name_map_df = edisgo_obj.topology.generators_df.loc[:, 'old_generator_name'].to_dict()

        generator_name_map = {}
        for i in range(0, len(generator_name_map_df.keys())):
            for j in range(0, len(list(generator_name_map_df.values())[i])):
                generator_name_map[list(generator_name_map_df.values())[i][j]] = \
        ...
        timeseries_generators_p_df = timeseries_generators_p_df.rename(index=rename_index)
        timeseries_generators_q_df = timeseries_generators_q_df.rename(index=rename_index)

        edisgo_obj.timeseries.generators_active_power = timeseries_generators_p_df.groupby(level=0).sum().T
        edisgo_obj.timeseries.generators_reactive_power = timeseries_generators_q_df.groupby(level=0).sum().T
    ...
    if not charging_points_df.empty and aggregate_charging_points_mode:
        ...
        charging_points_df = charging_points_df.groupby(by=['bus', 'use_case'], dropna=False).apply(aggregate_charging_points)

        edisgo_obj.topology.charging_points_df = charging_points_df

        charging_points_df.index = ('ChargingPoint_' + charging_points_df.loc[:, 'bus'].values + '_' + charging_points_df.loc[:, 'use_case'].values)

        logger.info('Aggregate charging points timeseries')
        timeseries_charging_points_p_df = edisgo_obj.timeseries.charging_points_active_power.T.copy()
        timeseries_charging_points_q_df = edisgo_obj.timeseries.charging_points_reactive_power.T.copy()

        charging_point_name_map_df = charging_points_df.loc[:, 'old_charging_point_name'].to_dict()
        ...
        for i in range(0, len(charging_point_name_map_df.keys())):
            for j in range(0, len(list(charging_point_name_map_df.values())[i])):
                charging_point_name_map[list(charging_point_name_map_df.values())[i][j]] = \
        ...
        timeseries_charging_points_p_df = timeseries_charging_points_p_df.rename(index=rename_index)
        timeseries_charging_points_q_df = timeseries_charging_points_q_df.rename(index=rename_index)

        timeseries_charging_points_p_df = timeseries_charging_points_p_df.groupby(level=0).sum().T
        timeseries_charging_points_q_df = timeseries_charging_points_q_df.groupby(level=0).sum().T

        edisgo_obj.timeseries.charging_points_active_power = timeseries_charging_points_p_df
        edisgo_obj.timeseries.charging_points_reactive_power = timeseries_charging_points_q_df
...
def make_busmap_from_clustering(edisgo_root=None,
                                grid=None,
                                mode=None,
    ...
    def calculate_weighting(series):
        p_gen = edisgo_obj.topology.generators_df.loc[
            edisgo_obj.topology.generators_df.bus == series.name, 'p_nom'].sum()
        p_load = edisgo_obj.topology.loads_df.loc[
            edisgo_obj.topology.loads_df.bus == series.name, 'peak_load'].sum()
        p_charge = edisgo_obj.topology.charging_points_df.loc[
            edisgo_obj.topology.charging_points_df.bus == series.name, 'p_nom'].sum()
        if str(grid).split('_')[0] == 'MVGrid':
            s_tran = edisgo_obj.topology.transformers_df.loc[
                edisgo_obj.topology.transformers_df.bus0 == series.name, 's_nom'].sum()
        else:
            s_tran = 0
    ...
    for grid in grid_list:
        ...
        if mode == 'kmeans':
            ...
            for index, new_bus in zip(partial_busmap_df.index,
                                      partial_busmap_df.new_bus):
                partial_busmap_df.loc[index, ['new_x', 'new_y']] = kmeans.cluster_centers_[new_bus]

        elif mode == 'kmeansdijkstra':
            dist_to_cluster_center = pd.DataFrame(data=kmeans.transform(buses_df.loc[:, ['x', 'y']]),
                                                  index=buses_df.index).min(axis='columns')

            buses_df.loc[:, 'cluster_number'] = kmeans.labels_
            medoid_bus_name = {}

            for n in range(0, n_clusters):
                medoid_bus_name[dist_to_cluster_center.loc[
                    buses_df.loc[:, "cluster_number"].isin([n])].idxmin()] = int(n)

            dijkstra_distances_df = pd.DataFrame(index=buses_df.index, columns=medoid_bus_name)
            ...
            for index in buses_df.index:
                partial_busmap_df.loc[index, 'new_bus'] = int(medoid_bus_name[buses_df.loc[index, 'medoid']])
                partial_busmap_df.loc[index, ['new_x', 'new_y']] = buses_df.loc[
                    buses_df.loc[index, 'medoid'], ['x', 'y']].values

        partial_busmap_df = partial_busmap_df.apply(rename_new_buses, axis='columns')

        partial_busmap_df.loc[partial_busmap_df.new_bus.isin([partial_busmap_df.loc[transformer_bus, 'new_bus']]), 'new_bus'] = transformer_bus

        if preserve_trafo_bus_coordinates:
            partial_busmap_df.loc[partial_busmap_df.new_bus.isin([partial_busmap_df.loc[transformer_bus, 'new_bus']]),
                          'new_x'] = buses_df.loc[transformer_bus, 'x']
            partial_busmap_df.loc[partial_busmap_df.new_bus.isin([partial_busmap_df.loc[transformer_bus, 'new_bus']]),
                          'new_y'] = buses_df.loc[transformer_bus, 'y']

        partial_busmap_df.index.name = 'old_bus'

        if str(grid).split('_')[0] == 'MVGrid':
            partial_busmap_df = rename_virtual_buses(logger, partial_busmap_df, transformer_bus)

        partial_busmap_df = partial_busmap_df.apply(transform_coordinates_back, axis='columns')
...
def make_busmap_from_feeders(edisgo_root=None,
                             grid=None,
                             mode=None,
                             reduction_factor=None,
                             focus_mode=False,
    ...
    def make_name(number_of_feeder_node):
        if number_of_feeder_node == 0:
            ...
            name = 'Bus_mvgd_' + str(mvgd_id) + '_F' + str(number_of_feeder) + '_B' + str(number_of_feeder_node)
        else:
            name = 'Bus_mvgd_' + str(mvgd_id) + '_lvgd_' + str(grid_id) + '_F' + str(number_of_feeder) + '_B' + str(number_of_feeder_node)

        return name

    def calculate_weighting(series):
        buses = partial_busmap_df.loc[partial_busmap_df.new_bus.isin([series.name])].index.tolist()
        p_gen = edisgo_obj.topology.generators_df.loc[edisgo_obj.topology.generators_df.bus.isin(buses), 'p_nom'].sum()
        p_load = edisgo_obj.topology.loads_df.loc[edisgo_obj.topology.loads_df.bus.isin(buses), 'peak_load'].sum()
        p_charge = edisgo_obj.topology.charging_points_df.loc[edisgo_obj.topology.charging_points_df.bus.isin(buses), 'p_nom'].sum()
        if str(grid).split('_')[0] == 'MVGrid':
            s_tran = edisgo_obj.topology.transformers_df.loc[
                edisgo_obj.topology.transformers_df.bus0 == series.name, 's_nom'].sum()
        else:
    ...
    edisgo_obj.topology.buses_df = edisgo_obj.topology.buses_df.apply(transform_coordinates, axis='columns')
    ...
    for grid in grid_list:
        ...
        transformer_node = grid.transformers_df.bus1.values[0]
        transformer_coordinates = grid.buses_df.loc[transformer_node, ['x', 'y']].tolist()
        logger.debug('Transformer node: {}'.format(transformer_node))

        neighbors = list(nx.neighbors(graph_root, transformer_node))
        logger.debug('Transformer neighbors has {} neighbors: {}'.format(len(neighbors), neighbors))
        ...
        for feeder_nodes in nx.connected_components(graph_without_transformer):
            ...
            number_of_distinct_nodes = feeder_buses_df.groupby(by=['x', 'y']).first().shape[0]
            logger.debug('Number_of_distinct_nodes = ' + str(number_of_distinct_nodes))
            ...
            if n_clusters == 0:
                for index in feeder_buses_df.index.tolist():
                    ...
                    partial_busmap_df.loc[index, ['new_x', 'new_y']] = transformer_coordinates
            else:
                kmeans = KMeans(n_clusters=n_clusters,
                                n_init=10,
                                random_state=42)
                kmeans.fit(feeder_buses_df.loc[:, ['x', 'y']], sample_weight=feeder_buses_df.loc[:, 'weight'])

                if mode == 'kmeans':
                    n = 0
                    for index in feeder_buses_df.index.tolist():
                        partial_busmap_df.loc[index, 'new_bus'] = make_name(kmeans.labels_[n] + 1)
                        partial_busmap_df.loc[index, ['new_x', 'new_y']] = kmeans.cluster_centers_[kmeans.labels_[n]]
                        n = n + 1
                elif mode == 'kmeansdijkstra':
                    dist_to_cluster_center = pd.DataFrame(data=kmeans.transform(feeder_buses_df.loc[:, ['x', 'y']]),
                                                          index=feeder_buses_df.index).min(axis='columns')
                    feeder_buses_df.loc[:, 'cluster_number'] = kmeans.labels_
                    medoid_bus_name = {}

                    for n in range(0, n_clusters):
                        medoid_bus_name[dist_to_cluster_center.loc[
                            feeder_buses_df.loc[:, "cluster_number"].isin([n])].idxmin()] = int(n)

                    dijkstra_distances_df = pd.DataFrame(index=feeder_buses_df.index, columns=medoid_bus_name)

                    for bus in medoid_bus_name:
                        path_series = pd.Series(
                            nx.single_source_dijkstra_path_length(graph_root, bus, cutoff=None, weight='length'))
                        dijkstra_distances_df.loc[:, bus] = path_series

                    feeder_buses_df.loc[:, 'medoid'] = dijkstra_distances_df.idxmin(axis=1)

                    for index in feeder_buses_df.index.tolist():
                        partial_busmap_df.loc[index, 'new_bus'] = make_name(medoid_bus_name[feeder_buses_df.loc[index, 'medoid']] + 1)
                        partial_busmap_df.loc[index, ['new_x', 'new_y']] = feeder_buses_df.loc[feeder_buses_df.loc[index, 'medoid'], ['x', 'y']].values
            number_of_feeder = number_of_feeder + 1

        if str(grid).split('_')[0] == 'MVGrid':
            partial_busmap_df = rename_virtual_buses(logger, partial_busmap_df, transformer_node)
...
def make_busmap_from_main_feeders(edisgo_root=None,
                                  grid=None,
                                  mode=None,
                                  reduction_factor=None,
                                  focus_mode=False,
    ...
    for grid in grid_list:
        ...
        for node in neighbors:
            path = nx.single_source_dijkstra_path_length(graph_without_transformer, node, weight='length')
            end_node = max(path, key=path.get)
            end_nodes.append(end_node)

        main_feeders_df = pd.DataFrame(columns=['distance',
        ...
        for end_node in end_nodes:
            distance, path = nx.single_source_dijkstra(graph_root,
        ...
        if mode == 'feeder_replacement_with_equidistant_nodes':
            def short_coordinates(root_node, end_node, branch_length, node_number):
                angle = math.degrees(math.atan2(end_node[1] - root_node[1], end_node[0] - root_node[0]))

                branch_length = 1000 * branch_length / 1.3

                x_new = root_node[0] + branch_length * node_number * math.cos(math.radians(angle))
                y_new = root_node[1] + branch_length * node_number * math.sin(math.radians(angle))
            ...
            for end_node in end_nodes:
                ...
                if focus_mode:
                    ...
                    for node in feeder.path[1:]:
                        buses = partial_busmap_df.loc[partial_busmap_df.new_bus.isin([node])].index.tolist()
                        for bus in buses:
                            if bus in buses_of_interest:
                                selected_reduction_factor = reduction_factor
                else:
                ...
                if selected_reduction_factor < 1:
                    nodes_per_feeder = math.ceil(selected_reduction_factor * feeder.number_of_nodes_in_path)
                else:
                ...
                for node in feeder.path:
                    distance_from_transformer = nx.shortest_path_length(graph_cleaned, source=transformer_node, target=node, weight='length')
                    old_feeder[node_number] = np.abs(new_feeder - distance_from_transformer).argmin()
                    node_number += 1

                # Make busmap for feeders
                end_coordinates = grid.buses_df.loc[end_nodes[number_of_feeder], ['x', 'y']].tolist()
                for node_number in range(0, old_feeder.shape[0]):
                    old_bus = feeder.path[node_number]
                    partial_busmap_df.loc[old_bus, 'new_bus'] = make_name(old_feeder[node_number])
                    coor = short_coordinates(transformer_coordinates,
                                             end_coordinates,
                                             branch_length,
        ...
        if mode != 'aggregate_to_longest_feeder':
            ...
            for node in not_main_nodes:
                partial_busmap_df.loc[node] = partial_busmap_df.loc[partial_busmap_df.loc[node, 'new_bus']]
...
def make_busmap(edisgo_root=None, mode=None, reduction_factor=None, grid=None):
    if mode == 'km':
        busmap_df = make_busmap_from_clustering(edisgo_root=edisgo_root,
        ...
        busmap_df = make_busmap_from_main_feeders(edisgo_root=edisgo_root,
                                                  mode='aggregate_to_longest_feeder',
                                                  grid=grid)
    elif mode == 'frwen':
        busmap_df = make_busmap_from_main_feeders(edisgo_root=edisgo_root,
                                                  reduction_factor=reduction_factor,
        ...
        busmap_df = make_busmap_from_feeders(edisgo_root=edisgo_root,
                                             reduction_factor=reduction_factor,
                                             grid=grid,
                                             mode='kmeans')
    elif mode == 'kmdpf':
...
def make_remaining_busmap(busmap_df, edisgo_root):
    ...
    remaining_busmap_df = edisgo_root.topology.buses_df.loc[
        ~edisgo_root.topology.buses_df.index.isin(busmap_df.index)].copy()
    remaining_busmap_df.loc[remaining_busmap_df.index, 'new_bus'] = remaining_busmap_df.index
...
def reduce_edisgo(edisgo_root, busmap_df, aggregate_charging_points_mode=True):
    ...
    buses_df = buses_df.groupby(by=['bus'],
                                dropna=False,
    ...
    def remove_lines_with_the_same_bus(series):
        if series.bus0 == series.bus1:
            return
        elif ((series.bus0.split('_')[0] == 'virtual'
               and series.bus0.lstrip('virtual_') == slack_bus)
              or (series.bus1.split('_')[0] == 'virtual'
                  and series.bus1.lstrip('virtual_') == slack_bus)
              ):
            logger.debug('Drop line because it is connected to the virtual_slack bus \n{}'.format(series.name))

            return
        elif series.bus0.lstrip('virtual_') == series.bus1.lstrip('virtual_'):
            logger.debug('Drop line because it shorts the circuit breaker \n{}'.format(series.name))
    ...
    def aggregate_lines_df(df):
        ...
        coordinates_bus0 = edisgo_obj.topology.buses_df.loc[bus0, ['x', 'y']].values.tolist()
        coordinates_bus1 = edisgo_obj.topology.buses_df.loc[bus1, ['x', 'y']].values.tolist()

        coordinates_bus0 = coor_transform.transform(coordinates_bus0[0],
                                                    coordinates_bus0[1])
        coordinates_bus1 = coor_transform.transform(coordinates_bus1[0],
                                                    coordinates_bus1[1])

        length = math.dist(coordinates_bus0, coordinates_bus1) / 1000 * edisgo_obj.config['grid_connection']['branch_detour_factor']

        if length == 0:
            length = 0.001
            logger.warning('Length of line between ' + str(bus0) + ' and ' + str(bus1) + ' can\'t be 0, set to 1m')
        if length < 0.001:
            logger.warning('WARNING: Length of line between ' + str(bus0) + ' and ' + str(bus1) + ' smaller than 1m')
        ...
        if len(df['type_info'].values) > 1:
            ...
            type_info = edisgo_obj.config['grid_expansion_standard_equipment'][type_line]
        else:
        ...
        for line_type in df['type_info'].values:
            x_sum = x_sum + 1 / line_data_df.loc[line_data_df.U_n.isin([v_nom])].loc[
                line_type, 'L_per_km']
        x_sum = 1 / x_sum
        ...
        r_sum = 0
        for line_type in df['type_info'].values:
            r_sum = r_sum + 1 / line_data_df.loc[line_data_df.U_n.isin([v_nom])].loc[
                line_type, 'R_per_km']
        r_sum = 1 / r_sum
        r = length * r_sum
    ...
    if not lines_df.empty:
        ...
        line_data_df = pd.concat([edisgo_obj.topology.equipment_data['mv_overhead_lines'],
        ...
        lines_df = lines_df.apply(remove_lines_with_the_same_bus, axis=1, result_type='broadcast').dropna()
    ...
    if not loads_changed_df.empty:
        ...
        if load_aggregation_mode == 'sector':
            loads_changed_df = loads_changed_df.groupby(by=['bus', 'sector']).apply(aggregate_loads)
            loads_changed_df.index = 'Load_' + loads_changed_df.loc[:, 'bus'] + '_' + loads_changed_df.loc[:, 'sector']
        elif load_aggregation_mode == 'bus':
    ...
    if not generators_changed_df.empty:
        ...
        generators_changed_df = generators_changed_df.apply(apply_busmap, axis='columns')

        if generator_aggregation_mode == 'bus':
            generators_changed_df = generators_changed_df.groupby('bus').apply(
                aggregate_generators_df)
            generators_changed_df.index = 'Generator_' + generators_changed_df.loc[:, 'bus']
        elif generator_aggregation_mode == 'type':
            generators_changed_df = generators_changed_df.groupby(
                by=['bus', 'type', 'weather_cell_id'], dropna=False).apply(aggregate_generators_df)
            generators_changed_df.index = ('Generator_' + generators_changed_df.loc[:, 'bus'].values + '_'
                                           + generators_changed_df.loc[:, 'type'].values + '_weather_cell_id_'
                                           + generators_changed_df.loc[:, 'weather_cell_id'].apply(extract_weather_cell_id).values)
        ...
        edisgo_obj.timeseries.generators_active_power = timeseries_generators_p_df.groupby(
            level=0).sum().T

        edisgo_obj.timeseries.generators_reactive_power = timeseries_generators_q_df.groupby(
            level=0).sum().T
    ...
    if not charging_points_df.empty:
        ...
        if aggregate_charging_points_mode:
            charging_points_df = charging_points_df \
                .groupby(by=['bus', 'use_case'], dropna=False) \
                .apply(aggregate_charging_points_df)

        edisgo_obj.topology.charging_points_df = charging_points_df

        if aggregate_charging_points_mode:
            charging_points_df.index = ('ChargingPoint_' + charging_points_df.loc[:, 'bus'].values
                                        + '_' + charging_points_df.loc[:, 'use_case'].values)

            timeseries_charging_points_p_df = edisgo_obj.timeseries.charging_points_active_power.T
            timeseries_charging_points_q_df = edisgo_obj.timeseries.charging_points_reactive_power.T
    ...
    for index, new_bus in zip(busmap_df.index, busmap_df.new_bus):
        if (index.split('_')[0] == 'virtual') and (new_bus.split('_')[0] != 'virtual'):
            switches_to_drop.append(switches_df.loc[switches_df.bus_open == index].index[0])
    ...
    # apply busmap
    for index, bus_open, bus_closed in zip(switches_df.index,
                                           switches_df.bus_open,
                                           switches_df.bus_closed):
    ...
    for index, bus_open, bus_closed in zip(switches_df.index,
        ...
        if lines_df.bus0.isin([bus_open]).any():
            switches_df.loc[index, 'branch'] = lines_df.loc[lines_df.bus0.isin([bus_open])].index[0]
        if lines_df.bus1.isin([bus_open]).any():
            switches_df.loc[index, 'branch'] = lines_df.loc[lines_df.bus1.isin([bus_open])].index[0]

    # remove duplicate switches_df
    switches_df = switches_df.groupby(by=['bus_closed', 'bus_open', 'branch'],
                                      as_index=False).first()
...
def save_results_reduced_to_min_max(edisgo_root, edisgo_object_name):
    ...
    edisgo_obj.save(edisgo_object_name,
                    save_results=True,
                    save_topology=True,
...
def voltage_mapping(edisgo_root, edisgo_reduced, busmap_df, timestep):
    ...
    for index, row in voltages_df.iterrows():
        try:
            voltages_df.loc[index, 'v_reduced'] = v_reduced.loc[busmap_df.loc[index, 'new_bus']]
            voltages_df.loc[index, 'new_bus_name'] = busmap_df.loc[index, 'new_bus']
        except KeyError:
            voltages_df.loc[index, 'v_reduced'] = v_reduced.loc[busmap_df.loc[index, 'new_bus'].lstrip('virtual_')]
            voltages_df.loc[index, 'new_bus_name'] = busmap_df.loc[index, 'new_bus'].lstrip('virtual_')
    voltages_df.loc[:, 'v_diff'] = voltages_df.loc[:, 'v_root'] - voltages_df.loc[:, 'v_reduced']
    rms = np.sqrt(mean_squared_error(voltages_df.loc[:, 'v_root'], voltages_df.loc[:, 'v_reduced']))
    logger.info('Root mean square value between edisgo_root voltages and edisgo_reduced: v_rms = {:.2%}'.format(rms))
...
def line_apparent_power_mapping(edisgo_root, edisgo_reduced, linemap_df, timestep):
    ...
    logger.info('Root mean square value between edisgo_root s_res and edisgo_reduced: s_rms = {:.2}'.format(rms))
...
def make_grid_list(edisgo_obj, grid):
    if edisgo_obj is None and grid is None:
        ...
        grid_name_list = grid_name_list + list(map(str, edisgo_obj.topology.mv_grid.lv_grids))
        grid_list = [edisgo_obj.topology.mv_grid]
        grid_list = grid_list + list(edisgo_obj.topology.mv_grid.lv_grids)
...
def rename_virtual_buses(logger, partial_busmap_df, transformer_node):
    ...
    for node in nodes:
        if node.split('_')[0] == 'virtual':
            if partial_busmap_df.loc[node, 'new_bus'] != partial_busmap_df.loc[node.lstrip('virtual_'), 'new_bus']:
    ...
    if len(pairs) > 0:
        ...
        for feeder_virtual, feeder_non_virtual in pairs:
            ...
            nodes_in_the_same_cluster_as_virtual_node = partial_busmap_df.loc[partial_busmap_df.loc[:, 'new_bus'].isin([old_name_of_virtual_node])].index.tolist()
            ...
            for nodes_to_add_a_virtual in nodes_in_the_same_cluster_as_virtual_node:
                ...
                if (partial_busmap_df.loc[feeder_non_virtual, 'new_bus'] != transformer_node)\
                        and (partial_busmap_df.loc[nodes_to_add_a_virtual, 'new_bus'] != transformer_node):

                    partial_busmap_df.loc[nodes_to_add_a_virtual, 'new_bus'] = 'virtual_' + partial_busmap_df.loc[feeder_non_virtual, 'new_bus']
    return partial_busmap_df



# Functions to make life easier
def setup_logger(stream_level=logging.INFO, file_level=logging.DEBUG, filename='edisgo.log', root=False):
    import sys
